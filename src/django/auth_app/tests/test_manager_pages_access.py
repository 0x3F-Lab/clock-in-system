...
@pytest.mark.django_db
def test_employee_cannot_access_report_generation_page(logged_in_employee):
    """
    Ensure that a non-manager user is redirected to the home page when
    attempting to access the report generation page.
    """
    response = logged_in_employee.get(reverse("report_generation"))
